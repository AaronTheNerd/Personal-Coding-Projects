...
#ifndef _TREE_HUFFMAN_TREE_H_
#define _TREE_HUFFMAN_TREE_H_
#include <string>
#include <map>
#include "Tree.h"


class Huffman : public Tree<std::string> {
 public:
  Huffman();                             // Default tree constructor
  ~Huffman();                            // Tree destructor
  void Add(std::string);                 // Adds text to the tree
  void Remove(std::string);              // Does nothing
 ...
 protected:
  ...
  std::string GetCodeOf(Node*, std::string);
